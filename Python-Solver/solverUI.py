...
class App():
    def __init__(self,main):
        ...
        self.text.pack(side=BOTTOM,fill=tk.Y)
    

        #Labels & Entries
        self.Word=tk.Label(self.midFrame1,borderwidth='218',text='Word:',bd=5,bg='white',font=('Adobe Arabic',14,''))
        self.Word.place(relx=0.1,rely=0.5,anchor=CENTER)
        self.word=tk.StringVar(self.midFrame1)
        self.wordEntry=tk.Entry(self.midFrame1,bd=4,textvariable=self.word).place(relx=0.2,rely=0.3)
        
        self.Colors=tk.Label(self.midFrame2,borderwidth='218',text='Colors:',bd=5,bg='white',font=('Adobe Arabic',14,''))
        self.Colors.place(relx=0.1,rely=0.5,anchor=CENTER)
        self.color=tk.StringVar(self.midFrame2)
        self.colorEntry=tk.Entry(self.midFrame2,bd=4,textvariable=self.color).place(relx=0.2,rely=0.3)

        self.enterButton=tk.Button(self.midFrame2,text='Enter',command=self.findPossibleResults).place(relx=0.8,rely=0.5)
